...
new_watermark = spark.sql("SELECT MAX(loaded_at) FROM lh_silver.dbo.openaq_measurements").collect()[0][0]
...
STG_TABLE = "lh_silver.stg.fct_air_quality"
PARAM_TABLE = "wh_gold.dbo.dm_air_measurement"

silver_meas = spark.read.table("lh_silver.dbo.openaq_measurements")
...
gold = (
    new_data
    .groupBy("zone_id", "measurement_date", "parameter")
    .agg(
    ...
    .withColumn(
        "date_key",
        F.date_format("measurement_date", "yyyyMMdd").cast("int")
    )
    .withColumnRenamed("parameter", "p_name")
    .drop("measurement_date")
    .join(
        measurements.select("p_id", "p_name"),
        "p_name",
        "left"
    )
    .drop("p_name")
