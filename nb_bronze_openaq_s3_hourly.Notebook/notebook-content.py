...
SOURCE_NAME = notebookutils.variableLibrary.getLibrary('bronze_source_names').openaq_hourly
...
TARGET_TABLE = "lh_bronze.dbo.openaq_measurements_hourly"

TARGET_PARAMS = {"pm25", "no2", "o3", "co", "pm10"}
...
def filter_existing_paths(paths: list[str]) -> list[str]:
    ...
    existing = []
    for p in paths:
        jpath = jvm.org.apache.hadoop.fs.Path(p)
...
def read_and_aggregate(paths: list[str], target_sensor_ids: set[int]):
    ...
    filtered = (
        ...
        .withColumn("hour_utc", F.hour("ts_utc"))
    )
    hourly = (
        filtered.groupBy("sensors_id", "location_id", "parameter", "units", "date_utc", "hour_utc")
        .agg(
            F.avg("value").alias("value"),
            F.count("value").cast("int").alias("reading_count"),
        )
        .withColumnRenamed("sensors_id", "sensor_id")
        .select(
            "sensor_id", "location_id", "parameter", "units",
            F.col("date_utc").cast("string").alias("date_utc"),
            "hour_utc", "value", "reading_count",
        )
    )
    return hourly
...
for partition in partition_keys:
    ...
    paths = filter_existing_paths(paths)
    ...
    try:
        if not paths or not target_sensor_ids:
            ...
            'status' : 'succeeded',
            'rows_written' : 0,
            'error_message': json.dumps({"note": "no existing S3 paths for this month"})
            })
            continue
...
mssparkutils.notebook.exit(json.dumps({
    ...
    "log_msg": f"OpenAQ S3 hourly bronze: {len(succeeded)} ok, {len(failed_p)} failed",
}))
