...
new_watermark = spark.sql("SELECT MAX(loaded_at) FROM lh_silver.dbo.gdp_yearly").collect()[0][0]
...
STG_TABLE = "lh_silver.stg.fct_gdp_yearly"

new_data = (
    spark.table("lh_silver.dbo.gdp_yearly")
    .filter(
...
gold = new_data.select(F.col("year"),F.col("country_code"),F.col("gdp_usd"))
