...
new_watermark = spark.sql(
    "SELECT MAX(loaded_at) FROM lh_silver.dbo.taxi_trips"
).collect()[0][0]
...
STG_TABLE = "lh_silver.stg.fct_taxi_hourly"
...
gold = (silver
    .filter((F.col("loaded_at") > last_processed_ts) & (F.col("loaded_at") <= new_watermark))
    .groupBy("trip_date", "hour_of_day", "pulocationid", "payment_type").agg(
...
gold = (gold
    .withColumn("total_passengers", F.col("total_passengers").cast("int"))
